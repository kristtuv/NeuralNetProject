...
def best_architecture():
    ...
    for node in nodes:
        for sample in n_samples:
            ...
            for lamb in lambs:
                ...
                nn = NeuralNet( X_train, Y_train, nodes = [X.shape[1],  node, 2],
                        activations = ['relu', None], cost_func='log')
                nn_l1 = NeuralNet( X_train, Y_train, nodes = [X.shape[1],  node, 2],
                        activations = ['relu', None], cost_func='log', regularization='l1', lamb=lamb)
                nn_l2 = NeuralNet( X_train, Y_train, nodes = [X.shape[1],  node, 2],
                        activations = ['relu', None], cost_func='log', regularization='l2', lamb=lamb)
        ...
        print('Nodes: %g' %node)
        print('OLS', 'L1', 'L2')
        ols_max = np.max(accuracies['ols']['Test'])
        ols_lambda = lambs.index(ols_max)
        l1_max = np.max(accuracies['l1']['Test'])
        l1_lambda = lambs.index(l1_max)
        l2_max = np.max(accuracies['l2']['Test'])
        l2_lambda = lambs.index(l2_max)
        print(ols_max, l1_max, l2_max)
